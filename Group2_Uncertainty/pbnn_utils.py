    ...
    def make_model(self):
        ...
        posterior_fn = tfp.layers.default_mean_field_normal_fn(
              loc_initializer=tf.random_normal_initializer(
                  mean=self.PRIOR_MU, stddev=0.05),
              untransformed_scale_initializer=tf.random_normal_initializer(
                  mean=np.log(np.exp(0.001) - 1), stddev=0.05))

        def prior_fn(dtype, shape, name, trainable, add_variable_fn):
            dist = tfp.distributions.Normal(loc=self.PRIOR_MU*tf.ones(shape, dtype),
                                 scale=self.PRIOR_SIGMA*tf.ones(shape, dtype))
        ...
        encoder_1_a = tfp.layers.Convolution2DFlipout(self.FILTERS, name='encoder_1_a', **flipout_params)(input_layer)
...
class DataLoader:
    def __init__(self):
        ...
        self.scratch_path = '/scratch/bdsi_root/bdsi1/scolando/'
