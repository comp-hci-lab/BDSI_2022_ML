...
#LAYER_PATH = ROOT_PATH + "/layers/" #+ LAYER_NAME + "/" #do not currently have this
#SAVE_PATH = LAYER_PATH + "_bayesian_model.h5" #no not currently have this
#PICKLE_PATH = ROOT_PATH + "/layers/" + '_hist.pkl'
...
Xy_train = tf.data.Dataset.zip((tf.data.Dataset.from_tensor_slices(np.load(DATA_PATH + "train_images.npy")), tf.data.Dataset.from_tensor_slices(np.load(DATA_PATH + 'train_labels.npy')))).cache().batch(2).prefetch(2)

#tf.data.Dataset.zip((tf.data.Dataset.from_tensor_slices(np.load(DATA_PATH + 'train_patients.npy')))).cache().batch(BATCH_SIZE).prefetch(2) #,
                               #tf.data.Dataset.from_tensor_slices(np.load(DATA_PATH + 'msks_train.npy')))).cache().batch(BATCH_SIZE).prefetch(2)
                                
Xy_test = tf.data.Dataset.zip((tf.data.Dataset.from_tensor_slices(np.load(DATA_PATH + 'test_images.npy')), tf.data.Dataset.from_tensor_slices(np.load(DATA_PATH +'test_labels.npy')))).cache().batch(2).prefetch(2) #,
